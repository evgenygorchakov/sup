...
- Type `/` to see commands (`Tab` completes): switch model, switch web search provider, toggle plan / auto / thinking / verbose output, set the sampling temperature (`/temp`), list saved plans, clear history; `Shift+Tab` cycles the mode in place.
...
- Tuned for small local models: tool arguments validated against the schema with precise repair errors, the approved plan re-injected near the end of the context every turn, old tool outputs collapsed.
- Sampling is two knobs and both can be handed back: `TEMPERATURE` ships at 0.2, which on the live suite ran ~20% faster than the model's own 1.0 at identical outcomes, and `/temp` changes it mid-session; `/temp default`, like an empty `TEMPERATURE`, sends nothing and lets the model's own value stand. `REPEAT_PENALTY` is empty by default — below 1.5 it measurably changes nothing.
